...
type T = {
  ...
  columns: {
    ...
    subtitle: string;
    enableAll: string;
    appearAfterPreview: React.ReactNode;
...
type PreviewRowCellProps = {
  value: string | null;
  rowNumber: number;
  idx: number;
  styles?: Array<import('./sheetStyleUtils').SheetCellStyle | null>;
};
const PreviewRowCell = ({
  value,
  rowNumber,
  idx,
  styles,
}: PreviewRowCellProps): React.JSX.Element => {
  ...
  return (
    <td key={`${rowNumber}-${idx}`} style={tdStyle}>
      {value ?? '—'}
    </td>
  );
};
      ...
      <table style={{ minWidth: '100%', fontSize: 12, borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ borderBottom: `1px solid ${c.ink150}` }}>
            ...
            {hasExtra && (
              <th style={{ padding: '8px', textAlign: 'left', fontWeight: 600, color: c.ink500 }}>
                …
              </th>
            )}
          </tr>
        ...
        <tbody>
          {preview.sampleRows.map(r => (
            <SampleTableRow key={r.rowNumber} row={r} hasExtra={hasExtra} />
          ))}
        </tbody>
      </table>
    </Box>
  ...
  return (
    <tr style={{ borderBottom: isLast ? 'none' : `1px solid ${c.ink50}` }}>
      <td style={{ padding: '8px 12px' }}>
        <Checkbox
          ...
          onCheckedChange={checked =>
            onUpdate({ index: col.index, patch: { include: checked as boolean } })
          }
          className="h-5 w-5"
        />
      </td>
      ...
      <td style={{ padding: '8px 12px' }}>
        <input
          value={col.title}
          onChange={e => onUpdate({ index: col.index, patch: { title: e.target.value } })}
      ...
      <td style={{ padding: '8px 12px' }}>
        <select
          ...
          onChange={e =>
            onUpdate({ index: col.index, patch: { suggestedType: e.target.value as ColumnType } })
          }
      ...
      <table style={{ minWidth: '100%', fontSize: 14, borderCollapse: 'collapse' }}>
        <thead style={{ background: c.ink50, borderBottom: `1px solid ${c.ink150}` }}>
          <tr>
            ...
            <th
              style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 600, color: c.ink800 }}
            >
        ...
        <tbody>
          {Array.from(columns.entries()).map(([idx, c]) => (
            <ColumnRow
              key={c.index}
              col={c}
              isLast={idx === lastIdx}
...
export const ColumnsPanel = ({
  ...
  setColumns,
  t: tRaw,
}: ColumnsPanelProps): React.JSX.Element => {
  const t = tRaw as unknown as T;
  const { resolvedTheme } = useTheme();
  const c = resolvedTheme === 'dark' ? tokens.dark.color : tokens.color;
  return (
