...
const normalizers: Record<ColumnType, (v: unknown) => CustomTableCellValue> = {
  ...
  // Деньги — то же число, оформление живёт в ячейке, а не в значении.
  currency: v => (isNullish(v) ? null : Number(v)),
  // Формула вычисляется сервером при выдаче, поэтому правке не подлежит.
  formula: v => (isNullish(v) ? null : Number(v)),
  // Связь хранит идентификатор строки-цели, подпись приходит отдельно.
  relation: v => (isNullish(v) ? null : String(v)),
  ai: v => (v === null || v === undefined ? '' : String(v)),
};
