...
export class AddSubscriptionManagement1778200000000 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "subscriptions_review_status_enum" AS ENUM ('current', 'needs_review')`,
    );
    await queryRunner.query(
      `CREATE TYPE "subscriptions_risk_status_enum" AS ENUM ('none', 'price_changed', 'date_shifted', 'missing_charge')`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "subscriptions" ADD "review_status" "subscriptions_review_status_enum" NOT NULL DEFAULT 'current'`,
    );
    await queryRunner.query(
      `ALTER TABLE "subscriptions" ADD "risk_status" "subscriptions_risk_status_enum" NOT NULL DEFAULT 'none'`,
    );
    await queryRunner.query(`ALTER TABLE "subscriptions" ADD "cancellation_reason" text`);
    await queryRunner.query(
      `ALTER TABLE "subscriptions" ADD "realized_annual_savings" numeric(15,2) NOT NULL DEFAULT 0`,
    );
    await queryRunner.query(
      `ALTER TABLE "subscriptions" ADD CONSTRAINT "FK_subscriptions_owner" FOREIGN KEY ("owner_id") REFERENCES "users"("id") ON DELETE SET NULL`,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_subscriptions_workspace_owner" ON "subscriptions" ("workspace_id", "owner_id")`,
    );

    await queryRunner.query(
      `CREATE TYPE "subscription_decisions_decision_enum" AS ENUM ('owner_assigned', 'keep', 'review', 'cancelled', 'price_reduced')`,
    );
    await queryRunner.query(`
    ...
    `);
    await queryRunner.query(
      `CREATE INDEX "IDX_subscription_decisions_subscription_created" ON "subscription_decisions" ("subscription_id", "created_at")`,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_subscription_decisions_workspace_created" ON "subscription_decisions" ("workspace_id", "created_at")`,
    );
  }
